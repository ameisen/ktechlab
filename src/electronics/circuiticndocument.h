...
public:
	 ...
	 */
	Connector * createConnector( Node *node, Connector *con, const QPoint &pos2, QList<QPoint> *pointList = nullptr ) override;
	/**
	 ...
	 */
	Connector *createConnector( Connector *con1, Connector *con2, const QPoint &pos1, const QPoint &pos2, QList<QPoint> *pointList = nullptr ) override;
	/**
	 * Creates a connector between two nodes, and returns a pointer to it
	 * and adds the operation to the undo list
	 */
	Connector* createConnector( const QString &startNodeId, const QString &endNodeId, QList<QPoint> *pointList = nullptr ) override;
	/**
	 * Creates a connector from node1 to node2. If pointList is non-null, then the
	 * connector will be assigned those points
	 */
	virtual Connector *createConnector(Node *node1, Node *node2, QList<QPoint> *pointList = nullptr)
